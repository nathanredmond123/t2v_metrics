...
class Score(nn.Module):
    ...
    def forward(self,
        ...
        # Process scores
        scores = torch.zeros(len(choices)).to(self.device)
        # for i, image in enumerate(processed_images):
        #     scores[i] = self.model.forward([image] * len(texts), texts, **kwargs)
        #print("\n\n", texts, "    ", choices, "    ", images, "\n\n")
#        choices_enumerated = []
#        for k in range(len(choices)):
#            choices_enumerated.append(f'{k+1}')
        scores = self.model.forward(images, texts, choices, fps=None, kwargs=kwargs)
