...
#        print("if we ever see this, then it is breaking")
        if paths[0].lower().endswith('.png') or paths[0].lower().endswith('.jpg'):
            # frames = [Image.open(path).convert('RGB') for path in paths]
            for path in paths:
                processed_data.append({"type": "image", "image": path})
...
#        assert len(paths) == len(texts), "Number of paths and texts must match"
        ...
        question = [kwargs["question_template"].format(text) for text in texts]
        processed_data = self.load_images(paths, fps)
        
        lm_probs = []
        # for data, question in zip(processed_data, questions):
        processed_data.append({"type": "text", "text": question[0]})
        ...
        ans_token_ids = [self.processor.tokenizer.encode(ans)[0] for ans in possible_answers]
        lm_probs = [probs[0, ans_token_id].item() for ans_token_id in ans_token_ids]
